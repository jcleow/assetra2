...
describe("Financial Planning Utils", () => {
  describe("formatCurrency", () => {
    ...
    it("should format currency with compact option", () => {
      ...
      expect(formatCurrency(50_000, { compact: true })).toBe("$50.0K");
  ...
  describe("calculateNetWorth", () => {
    it("should calculate net worth correctly", () => {
      expect(calculateNetWorth(100_000, 50_000)).toBe(50_000);
      expect(calculateNetWorth(50_000, 100_000)).toBe(-50_000);
  ...
  describe("validateProjectionSettings", () => {
    ...
    it("should validate return rate", () => {
      ...
      expect(errors1).toContain(
        "Average return rate must be between -50% and 50%"
      );
  ...
  describe("calculateCompoundGrowth", () => {
    it("should calculate compound growth correctly", () => {
      ...
      expect(calculateCompoundGrowth(10_000, 0.05, 5)).toBeCloseTo(
        12_762.82,
        2
      );
  ...
  describe("calculateFutureValue", () => {
    it("should calculate future value of monthly contributions", () => {
      ...
      expect(result).toBeCloseTo(173_085.07, 2);
    });

    it("should handle zero return rate", () => {
      const result = calculateFutureValue(1000, 0, 10);
      expect(result).toBe(120_000); // 1000 * 12 * 10
    });
  });

  describe("getNetWorthColor", () => {
    it("should return appropriate colors for different net worth levels", () => {
      expect(getNetWorthColor(1_500_000)).toBe("text-green-600"); // Millionaire+
      expect(getNetWorthColor(750_000)).toBe("text-green-500"); // Half million+
      expect(getNetWorthColor(150_000)).toBe("text-blue-600"); // Six figures
      expect(getNetWorthColor(50_000)).toBe("text-blue-500"); // Positive
      expect(getNetWorthColor(-25_000)).toBe("text-yellow-600"); // Small negative
      expect(getNetWorthColor(-100_000)).toBe("text-red-600"); // Significant debt
    });
  });

  describe("getNetWorthStatus", () => {
    it("should return appropriate status for different net worth levels", () => {
      expect(getNetWorthStatus(1_500_000)).toBe("Millionaire! 🎉");
      expect(getNetWorthStatus(750_000)).toBe("Half Millionaire 💰");
      expect(getNetWorthStatus(150_000)).toBe("Six Figures 📈");
      expect(getNetWorthStatus(50_000)).toBe("Positive Net Worth ✅");
      expect(getNetWorthStatus(-25_000)).toBe("Building Wealth 🔨");
      expect(getNetWorthStatus(-100_000)).toBe("High Debt 📉");
    });
  });
  ...
  describe("formatConfirmationSummary", () => {
    it("should format changes summary correctly", () => {
      ...
      expect(summary).toBe(
        "Updating: Assets: $150,000, Monthly Income: $9,000"
      );
    });

    it("should handle single change", () => {
      const changes = { totalLiabilities: 250_000 };
      const summary = formatConfirmationSummary(changes);
