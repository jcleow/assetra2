...
export function generateProjectionSummary(
  ...
  const retirement =
    timeline.find((point) => point.age >= settings.retirementAge) ||
...
export function getNetWorthColor(netWorth: number): string {
  if (netWorth >= 1_000_000) return "text-green-600"; // Millionaire
  if (netWorth >= 500_000) return "text-green-500"; // Half million
  if (netWorth >= 100_000) return "text-blue-600"; // Six figures
  if (netWorth >= 0) return "text-blue-500"; // Positive
  if (netWorth >= -50_000) return "text-yellow-600"; // Small negative
...
export function getNetWorthStatus(netWorth: number): string {
  ...
  if (netWorth >= 500_000) return "Half Millionaire 💰";
  if (netWorth >= 100_000) return "Six Figures 📈";
  if (netWorth >= 0) return "Positive Net Worth ✅";
  if (netWorth >= -50_000) return "Building Wealth 🔨";
  return "High Debt 📉";
}
